...
func _update(_p_delta) :
	...
	if player.is_on_floor() || player.can_coyote_jump:
		if !Global.jump_lock && blackboard.get_var(BBNames.jump_var) && blackboard.get_var(BBNames.jumps_made_var) == 0:
			jump()
			if player.can_coyote_jump:
				player.can_coyote_jump = false
				print("coyote jump")
		elif blackboard.get_var(BBNames.volt_var) && Global.vault_unlocked:
			vault()
...
func move() -> Vector2:
	...
	if not is_zero_approx(direction.x):
		...
		Global.last_moved_dir = direction
...
func vault():
	player.velocity.y = player.vault_velocity
	dispatch("vault")
